...
public class PREditForm extends JDialog {
    ...
    private PurchaseRequisition originalPR;
    
    public PREditForm(JFrame parent, PurchaseRequisition pr, User user) {
        super(parent, "Edit Purchase Requisition - " + pr.getPrId(), true);
    ...
    private void initializeComponents() {
        ...
        gbc.gridx = 1;
        headerPanel.add(new JLabel(originalPR.getPrId()), gbc);
        
        gbc.gridx = 0; gbc.gridy = 1;
        headerPanel.add(new JLabel("Requested By:"), gbc);
        gbc.gridx = 1;
        headerPanel.add(new JLabel(originalPR.getRequestedBy()), gbc);
        
        gbc.gridx = 0; gbc.gridy = 2;
        headerPanel.add(new JLabel("Department:"), gbc);
        gbc.gridx = 1;
        departmentField = new JTextField(20);
        departmentField.setText(originalPR.getDepartment());
        ...
        priorityCombo.setSelectedItem(originalPR.getPriority());
        ...
        justificationArea.setText(originalPR.getJustification());
        ...
        JButton updateButton = new JButton("Update PR");
        updateButton.addActionListener(this::updatePR);
    ...
    private void loadPRData() {
        for (PurchaseRequisitionLine line : originalPR.getLines()) {
            Object[] rowData = {
                line.getItemCode(),
                line.getItemName(),
                line.getQuantity(),
    ...
    private void updatePR(ActionEvent e) {
        try {
            ...
            originalPR.setDepartment(department);
            originalPR.setPriority(priority);
            originalPR.setJustification(justification);
            ...
            for (int i = 0; i < lineTableModel.getRowCount(); i++) {
                ...
                PurchaseRequisitionLine line = new PurchaseRequisitionLine(
                    originalPR.getPrId(), itemId, itemCode, itemName, quantity, unit, price, specifications);
                lines.add(line);
            }
            
            originalPR.setLines(lines);
            
            if (prService.updatePR(originalPR) && prService.updatePRLines(originalPR.getPrId(), lines)) {
                JOptionPane.showMessageDialog(this, "Purchase Requisition updated successfully!");
                dispose();
            } else {
                JOptionPane.showMessageDialog(this, "Error updating PR.");
            }
            
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Error updating PR: " + ex.getMessage());
            ex.printStackTrace();
        }
    }
}